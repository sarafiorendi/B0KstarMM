...
unsigned int CTRLmisTagWRKflow;
...
// ##############################################################
{
  ...
  if ((CTRLmisTagWRKflow == 1) && (strcmp(CTRLfitWRKflow.c_str(),"trueAll&FitFrac") != 0))
...
void GenerateParameterFile (RooAbsPdf* TotalPDF, vector<vector<string>*>* fitParam, vector<vector<unsigned int>*>* configParam, RooArgSet* vecConstr, string fileName, unsigned int fileIndx, vector<double>* q2Bins, unsigned int q2BinIndx)
{
  ...
  if ((CTRLmisTagWRKflow == 1) && (GetVar(TotalPDF,"nMisTagFrac") != NULL))
    {
...
void InstantiateMassFit (RooAbsPdf** TotalPDF, RooRealVar* x, string fitName, vector<vector<unsigned int>*>* configParam, unsigned int q2BinIndx)
{
  ...
  if (CTRLmisTagWRKflow == 0) nMisTagFrac->setConstant(true);
...
int main(int argc, char** argv)
{
  if (argc >= 4)
	  ...
	  (((FitType == 56) || (FitType == 76))   && (argc == 4)))
	{
	  ...
	  CTRLmisTagWRKflow = atoi(Utility->GetGenericParam("CtrlMisTagWrkFlow").c_str());
	  ...
	  fileFitResults << "Control mis-tag fraction workflow: "      << CTRLmisTagWRKflow << endl;
