...
// # make softlink to the created libraries: ln -s Roo(name).so libRoo(name).so   #
...
#define GENPARAMS     "All_fitParam" // Option to generate p.d.f. parameters
...
void DrawString (double Lumi, RooPlot* myFrame)
{
  ...
  if (myFrame == NULL) LumiTex2->DrawLatex(0.28,0.9,myString.str().c_str());
  else
    {
      LumiTex2->Paint();
  ...
  if (myFrame == NULL) LumiTex3->DrawLatex(0.76,0.9,myString.str().c_str());
  else
    {
      LumiTex3->Paint();
...
// ###################
{
  ...
  if ((FitType == 1) || (FitType == 41) || (FitType == 61) || (FitType == 81) || // Branching fraction
      ...
      for (int i = 0; i < effFunc->GetNpar(); i++)
	{
      ...
      if (doTransf == true)
	{
	  FlS  = new RooRealVar("FlS","#it{F}_{L}",0.0);
	  AfbS = new RooRealVar("AfbS","#it{A}_{FB}",0.0);
	}
      else
	{
	  FlS  = new RooRealVar("FlS","#it{F}_{L}",0.0,0.0,1.0);
	  AfbS = new RooRealVar("AfbS","#it{A}_{FB}",0.0,-2./3.,2./3.);
	}
      VarsAng->add(*FlS);
      VarsAng->add(*AfbS);
      VarsAng->add(*y);
	  ...
	  myString << "(3/4 * (3/2 * " << Utility->Transformer("FlS",doTransf,a,b,c) << " * (1-" << y->getPlotLabel() << "*" << y->getPlotLabel() << ") * " << z->getPlotLabel() << "*" << z->getPlotLabel() << " + ";
    	...
    	{
	  FsS = new RooRealVar("FsS","#it{F}_{S}",0.0,0.0,1.0);
          AsS = new RooRealVar("AsS","#it{A}_{S}",0.0,-1.0,1.0);
    	  VarsAng->add(*FsS);
    	  VarsAng->add(*AsS);
      ...
      if (useEffPDF == true)
	{
      ...
      // else AnglesPDF = new RooAngleMisTagPdf("AngleMisTag","AngleMisTag",RooArgSet(*VarsAng));
    }
...
void ResetCombPolyParam (vector<vector<string>*>* fitParam, RooAbsPdf* pdf)
{
  ...
  if (fitParam != NULL)
	...
	{
	  cout << "[ExtractYield::ResetCombPolyParam]\t@@@ Resetting the combinatorial background polynomial coefficients for q^2 bin #" << j << " @@@" << endl;
	  ...
	  for (int i = 0; i < atoi(fitParam->operator[](Utility->GetFitParamIndx("nPolyC1"))->operator[](j).c_str()); i++)
	    {
	      ...
	      cout << myString.str() << "\t" << fitParam->operator[](Utility->GetFitParamIndx(myString.str().c_str()))->operator[](j).c_str() << endl;
	    }
    ...
    {
      cout << "[ExtractYield::ResetCombPolyParam]\t@@@ Resetting the combinatorial background polynomial coefficients in the p.d.f. @@@" << endl;
...
void ResetAngularParam (vector<vector<string>*>* fitParam, RooAbsPdf* pdf)
{
  ...
  if (fitParam != NULL)
    {      
      for (unsigned int j = 0; j < fitParam->operator[](0)->size(); j++)
	{
	  cout << "[ExtractYield::ResetAngularParam]\t@@@ Resetting the angular parameters for q^2 bin #" << j << " @@@" << endl;
    ...
    {
      cout << "[ExtractYield::ResetAngularParam]\t@@@ Resetting the angular parameters in the p.d.f. @@@" << endl;
...
// #########################
{
  ...
  if ((option.find("All") != string::npos) || (option.find("misTagFrac") != string::npos))
    {
      if ((atoi(Utility->GetGenericParam("CtrlMisTagWrkFlow").c_str()) == 0) && (GetVar(pdf,"nMisTagFrac") != NULL))
	{
	  ...
	  cout << "Mis-tag fraction generation : gaussian mean = " << GetVar(pdf,"nMisTagFrac")->getVal() << "\tsigma = " << GetVar(pdf,"nMisTagFrac")->getError() << "\tvalue = " << myString.str() << endl;

	  if      (option.find("fitParam") != string::npos) fitParam->operator[](Utility->GetFitParamIndx("nMisTagFrac"))->operator[](q2BinIndx) = myString.str();
	  else if (option.find("pdf")      != string::npos) SetValueAndErrors(pdf,"nMisTagFrac",1.0,&myString,&value,&errLo,&errHi);
	  else
	    {
	      cout << "[ExtractYield::GenerateFitParameters]\tIncorrect option parameter : " << option << endl;
	      exit (EXIT_FAILURE);
	    }
	}
    }
  ...
  if (option.find("All") != string::npos)
      ...
      if (GetVar(pdf,"var1") != NULL)
	{
	  ...
	  cout << "Background var1 generation : uniform lower bound = 0\thigher bound = 1" << "\tvalue = " << myString.str() << endl;

	  if      (option.find("fitParam") != string::npos) fitParam->operator[](Utility->GetFitParamIndx("var1"))->operator[](q2BinIndx) = myString.str();
	  else if (option.find("pdf")      != string::npos) SetValueAndErrors(pdf,"var1",1.0,&myString,&value,&errLo,&errHi);
	  else
	    {
	      cout << "[ExtractYield::GenerateFitParameters]\tIncorrect option parameter : " << option << endl;
	      exit (EXIT_FAILURE);
      ...
      if (GetVar(pdf,"var2") != NULL)
	{
	  myString.clear(); myString.str("");
	  myString << RooRandom::uniform();
	  cout << "Background var2 generation : uniform lower bound = 0\thigher bound = 1" << "\tvalue = " << myString.str() << endl;

	  if      (option.find("fitParam") != string::npos) fitParam->operator[](Utility->GetFitParamIndx("var2"))->operator[](q2BinIndx) = myString.str();
	  else if (option.find("pdf")      != string::npos) SetValueAndErrors(pdf,"var2",1.0,&myString,&value,&errLo,&errHi);
	  else
	    {
	      cout << "[ExtractYield::GenerateFitParameters]\tIncorrect option parameter : " << option << endl;
	      exit (EXIT_FAILURE);
      ...
      if (GetVar(pdf,"fracMassBExp") != NULL)
	{
	  myString.clear(); myString.str("");
	  myString << RooRandom::uniform();
	  cout << "Background fraction uniform generation : " << "\tvalue = " << myString.str() << endl;

	  if      (option.find("fitParam") != string::npos) fitParam->operator[](Utility->GetFitParamIndx("fracMassBExp"))->operator[](q2BinIndx) = myString.str();
	  else if (option.find("pdf")      != string::npos) SetValueAndErrors(pdf,"var3",1.0,&myString,&value,&errLo,&errHi);
	  else
	    {
	      cout << "[ExtractYield::GenerateFitParameters]\tIncorrect option parameter : " << option << endl;
	      exit (EXIT_FAILURE);
  ...
  if ((option.find("All") != string::npos) || (option.find("FlAfbFsAs") != string::npos) || (option.find("FlAfb") != string::npos))
    {
      if (GetVar(pdf,"FlS") != NULL)
	{
	  myString.clear(); myString.str("");
	  myString << RooRandom::uniform();
	  cout << "Fl generation : uniform lower bound = 0\thigher bound = 1" << "\tvalue = " << myString.str() << endl;

	  if      (option.find("fitParam") != string::npos) fitParam->operator[](Utility->GetFitParamIndx("FlS"))->operator[](q2BinIndx) = myString.str();
	  else if (option.find("pdf")      != string::npos) SetValueAndErrors(pdf,"FlS",1.0,&myString,&value,&errLo,&errHi);
	  else
	    {
	      cout << "[ExtractYield::GenerateFitParameters]\tIncorrect option parameter : " << option << endl;
	      exit (EXIT_FAILURE);
      ...
      if (GetVar(pdf,"AfbS") != NULL)
	{
	  ...
	  if      (option.find("fitParam") != string::npos) fitParam->operator[](Utility->GetFitParamIndx("AfbS"))->operator[](q2BinIndx) = myString.str();
	  else if (option.find("pdf")      != string::npos) SetValueAndErrors(pdf,"AfbS",1.0,&myString,&value,&errLo,&errHi);
	  else
	    {
	      cout << "[ExtractYield::GenerateFitParameters]\tIncorrect option parameter : " << option << endl;
	      exit (EXIT_FAILURE);
  ...
  if ((option.find("All") != string::npos) || (option.find("FlAfbFsAs") != string::npos))
    {
      if (GetVar(pdf,"FsS") != NULL)
	{
	  myString.clear(); myString.str("");
	  myString << RooRandom::uniform();
	  cout << "Fs generation : uniform lower bound = 0\thigher bound = 1" << "\tvalue = " << myString.str() << endl;

	  if      (option.find("fitParam") != string::npos) fitParam->operator[](Utility->GetFitParamIndx("FsS"))->operator[](q2BinIndx) = myString.str();
	  else if (option.find("pdf")      != string::npos) SetValueAndErrors(pdf,"FsS",1.0,&myString,&value,&errLo,&errHi);
	  else
	    {
	      cout << "[ExtractYield::GenerateFitParameters]\tIncorrect option parameter : " << option << endl;
	      exit (EXIT_FAILURE);
      ...
      if (GetVar(pdf,"AsS") != NULL)
	{
	  ...
	  cout << "As generation : uniform lower bound = -1\thigher bound = 1" << "\tvalue = " << myString.str() << endl;

	  if      (option.find("fitParam") != string::npos) fitParam->operator[](Utility->GetFitParamIndx("AsS"))->operator[](q2BinIndx) = myString.str();
	  else if (option.find("pdf")      != string::npos) SetValueAndErrors(pdf,"AsS",1.0,&myString,&value,&errLo,&errHi);
	  else
	    {
	      cout << "[ExtractYield::GenerateFitParameters]\tIncorrect option parameter : " << option << endl;
	      exit (EXIT_FAILURE);
  ...
  if (option.find("All") != string::npos)
      ...
      if (GetVar(pdf,"P1S") != NULL)
	{
	  ...
	  cout << "P1 generation : uniform lower bound = " << GetVar(pdf,"P1S")->getMin() << "\thigher bound = " << GetVar(pdf,"P1S")->getMax() << "\tvalue = " << myString.str() << endl;

	  if      (option.find("fitParam") != string::npos) fitParam->operator[](Utility->GetFitParamIndx("P1S"))->operator[](q2BinIndx) = myString.str();
	  else if (option.find("pdf")      != string::npos) SetValueAndErrors(pdf,"P1S",1.0,&myString,&value,&errLo,&errHi);
	  else
	    {
	      cout << "[ExtractYield::GenerateFitParameters]\tIncorrect option parameter : " << option << endl;
	      exit (EXIT_FAILURE);
      ...
      if (GetVar(pdf,"P2S") != NULL)
	{
	  ...
	  cout << "P2 generation : uniform lower bound = " << GetVar(pdf,"P2S")->getMin() << "\thigher bound = " << GetVar(pdf,"P2S")->getMax() << "\tvalue = " << myString.str() << endl;

	  if      (option.find("fitParam") != string::npos) fitParam->operator[](Utility->GetFitParamIndx("P2S"))->operator[](q2BinIndx) = myString.str();
	  else if (option.find("pdf")      != string::npos) SetValueAndErrors(pdf,"P2S",1.0,&myString,&value,&errLo,&errHi);
	  else
	    {
	      cout << "[ExtractYield::GenerateFitParameters]\tIncorrect option parameter : " << option << endl;
	      exit (EXIT_FAILURE);
  ...
  if ((option.find("All") != string::npos) || (option.find("combBkgAng") != string::npos))
    {
      for (unsigned int i = 0; i < NCoeffPolyBKGcomb1; i++)
	{
	  myCoeff.clear(); myCoeff.str("");
	  ...
	  if (GetVar(pdf,myCoeff.str().c_str()) != NULL)
	    {
	      ...
	      cout << myCoeff.str() << " generation : gaussian mean = " << GetVar(pdf,myCoeff.str().c_str())->getVal() << "\tsigma = " << GetVar(pdf,myCoeff.str().c_str())->getError() << "\tvalue = " << myString.str() << endl;

	      if      (option.find("fitParam") != string::npos) fitParam->operator[](Utility->GetFitParamIndx(myCoeff.str().c_str()))->operator[](q2BinIndx) = myString.str();
	      else if (option.find("pdf")      != string::npos) SetValueAndErrors(pdf,myCoeff.str().c_str(),1.0,&myString,&value,&errLo,&errHi);
	      else
		{
		  cout << "[ExtractYield::GenerateFitParameters]\tIncorrect option parameter : " << option << endl;
		  exit (EXIT_FAILURE);
...
string GeneratePolynomial (RooRealVar* var, unsigned int nCoef, string sCoef)
{
  ...
  cout << "[ExtractYield::GeneratePolynomial]\tI've generated the polynomial: " << myString.str() << endl;
  ...
  if (justPlotMuMuMass == true)
    {
      ...
      cout << "[ExtractYield::FitDimuonInvMass]\tCut for dimuon invariant mass : " << myString.str() << endl;
  ...
  else
    {
      ...
      (*TotalPDFJPsi)->plotOn(myFrameJPsi, Components(*JPsi), FillStyle(3345), FillColor(kBlue), LineStyle(7), LineColor(kBlue), DrawOption("F"));
      ...
      if (justKeepPsi != true)
	{
	  ...
	  else if (legNamesPsi[it] == "Signal") legJPsi->AddEntry(obj,legNamesPsi[it++],"PFL");
      ...
      (*TotalPDFPsiP)->plotOn(myFramePsiP, Components(*PsiP), FillStyle(3345), FillColor(kBlue), LineStyle(7), LineColor(kBlue), DrawOption("F"));
      legNamesPsi[nElements++] = "Signal";
      ...
      for (unsigned int i = 0; i < 2*nElements; i++)
	{
	  ...
	  else if (legNamesPsi[it] == "Signal") legPsiP->AddEntry(obj,legNamesPsi[it++],"PFL");
...
void MakeDatasets (B0KstMuMuSingleCandTreeContent* NTuple, unsigned int FitType, vector<double>* q2Bins)
{
  ...
  if (!(((FitType >= 21) && (FitType <= 26)) || ((FitType >= 81) && (FitType <= 86)) || (FitType == 96)))
      ...
      for (int entry = 0; entry < nEntries; entry++)
	{
      ...
      cout << "[ExtractYield::MakeDatasets]\t@@@ NTuple with all data @@@" << endl;
      SingleCandNTuple->Print("v");

      cout << "[ExtractYield::MakeDatasets]\t@@@ NTuple with J/psi region @@@" << endl;
      SingleCandNTuple_JPsi->Print("v");
      
      cout << "[ExtractYield::MakeDatasets]\t@@@ NTuple with psi(2S) region @@@" << endl;
      ...
      cout << "[ExtractYield::MakeDatasets]\t@@@ NTuple with just J/psi and psi(2S) regions @@@" << endl;
  ...
  if (GetVar(*TotalPDF,"nSig") != NULL)
    {
      ...
      (*TotalPDF)->plotOn(myFrameX, Components(*MassSignal), FillStyle(3345), FillColor(kBlue), LineStyle(7), LineColor(kBlue), DrawOption("F"));
  ...
  if (GetVar(*TotalPDF,"nMisTagFrac") != NULL)
    {
      ...
      (*TotalPDF)->plotOn(myFrameX, Components(*MassMisTag), FillStyle(3354), FillColor(kGreen+1), LineStyle(8), LineColor(kGreen+1), DrawOption("F"));
  ...
  for (unsigned int i = 0; i < 2*nElements; i++)
    {
      ...
      else if ((legNames[it] == "Corr. tag sig.") || (legNames[it] == "Mistag sig.")) legX->AddEntry(obj,legNames[it++],"PFL");
  ...
  if (useEffPDF == true)
    {
      ...
      cout << "[ExtractYield::IterativeMassFitq2Bins]\t@@@ Integral of S*E over angular variables for normalization channel good-tagged events : " << effPsiGoodTag << " +/- " << effPsiGoodTagErr << " @@@" << endl;
      ...
      cout << "[ExtractYield::IterativeMassFitq2Bins]\t@@@ Integral of S*E over angular variables for normalization channel mis-tagged events : " << effPsiMisTag << " +/- " << effPsiMisTagErr << " @@@" << endl;
  ...
  for (unsigned int i = (specBin == -1 ? 0 : specBin); i < (specBin == -1 ? q2Bins->size()-1 : specBin+1); i++)
    {
      ...
      cout << "[ExtractYield::IterativeMassFitq2Bins]\tCut string: " << myString.str() << endl;
      ...
      for (int j = 0; j < static_cast<int>(dataSet_q2Bins[i]->sumEntries()); j++)
	{
      ...
      if (CheckGoodFit(fitResult) == true) cout << "[ExtractYield::IterativeMassFitq2Bins]\t@@@ Fit converged ! @@@" << endl;
      else                                 cout << "[ExtractYield::IterativeMassFitq2Bins]\t@@@ Fit didn't converge ! @@@" << endl;
      ...
      if (useEffPDF == true)
	{
	  ...
	  cout << "[ExtractYield::IterativeMassFitq2Bins]\t===> Integral of S*E over angular variables for signal good-tagged events <===" << endl;
	  ...
	  cout << "[ExtractYield::IterativeMassFitq2Bins]\t===> Integral of S*E over angular variables for signal mis-tagged events <===" << endl;
...
void MakeMassToy (RooAbsPdf* TotalPDF, RooRealVar* x, TCanvas* Canv, unsigned int nToy, int specBin, vector<vector<string>*>* fitParam, RooArgSet* vecConstr, string fileName)
{
  ...
  cout << "[ExtractYield::MakeMassToy]\t@@@ I'm generating the toys @@@" << endl;
  if (FULLTOYS == true)
    {
  ...
  cout << "[ExtractYield::MakeMassToy]\t@@@ Now fit total TOY invariant mass @@@" << endl;
  ...
  for (unsigned int i = 0; i < nToy; i++)
    {
      cout << "[ExtractYield::MakeMassToy]\t@@@ Now fitting toy #" << i << " @@@" << endl;
      ...
      do
	{
	  ...
	  if (CheckGoodFit(fitResult) == true) cout << "[ExtractYield::MakeMassToy]\t@@@ Fit converged ! @@@" << endl;
	  else
	    {
	      cout << "[ExtractYield::MakeMassToy]\t@@@ Fit didn't converge (attempt " << itTrials << "/" << TOYMULTYATTEMPTS << ") ! @@@" << endl;
	      GenerateFitParameters(TotalPDF,fitParam,specBin,"All_pdf");
	      itTrials++;
	    }
  ...
  if (SAVEPLOT == true)
    {
      cout << "[ExtractYield::MakeMassToy]\t@@@ Saving histograms @@@" << endl;
...
RooFitResult* MakeMass2AnglesFit (RooDataSet* dataSet, RooAbsPdf** TotalPDF, RooRealVar* x, RooRealVar* y, RooRealVar* z, TCanvas* Canv, unsigned int FitType, RooArgSet* vecConstr, double* NLLvalue, TPaveText* extText, unsigned int ID)
{
  ...
  if ((FitType == 36) || (FitType == 56) || (FitType == 76))
      ...
      for (unsigned int i = 0; i < 2*nElements; i++)
	{
	  ...
	  else if ((legNames[it] == "Corr. tag sig.") || (legNames[it] == "Mistag sig.")) legY->AddEntry(obj,legNames[it++],"PFL");
      ...
      for (unsigned int i = 0; i < 2*nElements; i++)
	{
	  ...
	  else if ((legNames[it] == "Corr. tag sig.") || (legNames[it] == "Mistag sig.")) legZ->AddEntry(obj,legNames[it++],"PFL");
  ...
  else
    {
	  ...
	  myString << " || B0MassArb > " << GetVar(*TotalPDF,"meanS")->getVal() + atof(Utility->GetGenericParam("NSigmaB0").c_str())*Utility->GetB0Width();
      	  cout << "[ExtractYield::MakeMass2AnglesFit]\tCut for B0 sidebands : " << myString.str() << endl;
	  ...
	  if (CheckGoodFit(fitResult) == true) { cout << "[ExtractYield::MakeMass2AnglesFit]\t@@@ Fit converged ! @@@" << endl;       delete fitResult; }
	  else                                 { cout << "[ExtractYield::MakeMass2AnglesFit]\t@@@ Fit didn't converge ! @@@" << endl; return fitResult; }
	}
      ...
      if (GetVar(*TotalPDF,"nSig") != NULL)
	{
	  ...
	  (*TotalPDF)->plotOn(myFrameX, Components(*Signal), FillStyle(3345), FillColor(kBlue), Project(RooArgSet(*y,*z)), LineStyle(7), LineColor(kBlue), DrawOption("F"));
	  legNames[nElements++] = "Corr. tag sig.";
	  VarsYield.add(*nSig);
	}
      ...
      if (GetVar(*TotalPDF,"nMisTagFrac") != NULL)
	{
	  ...
	  (*TotalPDF)->plotOn(myFrameX, Components(*MassAngleMisTag), FillStyle(3354), FillColor(kGreen+1), Project(RooArgSet(*y,*z)), LineStyle(8), LineColor(kGreen+1), DrawOption("F"));
	  legNames[nElements++] = "Mistag sig.";
	  VarsYield.add(*nMisTagFrac);
	}
      ...
      if (GetVar(*TotalPDF,"nSig")        != NULL)
	{
	  ...
	  (*TotalPDF)->plotOn(myFrameY, Components(*Signal), FillStyle(3345), FillColor(kBlue), Project(RooArgSet(*x,*z)), LineStyle(7), LineColor(kBlue), DrawOption("F"));
	}
      if (GetVar(*TotalPDF,"nMisTagFrac") != NULL)
	{
	  (*TotalPDF)->plotOn(myFrameY, Components(*MassAngleMisTag), LineStyle(8),    LineColor(kGreen+1), Project(RooArgSet(*x,*z)), DrawOption("L"));
	  (*TotalPDF)->plotOn(myFrameY, Components(*MassAngleMisTag), FillStyle(3354), FillColor(kGreen+1), Project(RooArgSet(*x,*z)), LineStyle(8), LineColor(kGreen+1), DrawOption("F"));
	}
      ...
      if (GetVar(*TotalPDF,"nSig")        != NULL)
	{
	  ...
	  (*TotalPDF)->plotOn(myFrameZ, Components(*Signal), FillStyle(3345), FillColor(kBlue), Project(RooArgSet(*x,*y)), LineStyle(7), LineColor(kBlue), DrawOption("F"));
	}
      if (GetVar(*TotalPDF,"nMisTagFrac") != NULL)
	{
	  (*TotalPDF)->plotOn(myFrameZ, Components(*MassAngleMisTag), LineStyle(8),    LineColor(kGreen+1), Project(RooArgSet(*x,*y)), DrawOption("L"));
	  (*TotalPDF)->plotOn(myFrameZ, Components(*MassAngleMisTag), FillStyle(3354), FillColor(kGreen+1), Project(RooArgSet(*x,*y)), LineStyle(8), LineColor(kGreen+1), DrawOption("F"));
	}
      ...
      if (GetVar(*TotalPDF,"nSig")        != NULL)
	{
	  ...
	  (*TotalPDF)->plotOn(myFrameLowSideBY, Components(*Signal), FillStyle(3345), FillColor(kBlue), ProjectionRange("lowSideband"), Project(RooArgSet(*x,*z)), LineStyle(7), LineColor(kBlue), DrawOption("F"));
	}
      if (GetVar(*TotalPDF,"nMisTagFrac") != NULL)
	{
	  ...
	  (*TotalPDF)->plotOn(myFrameLowSideBY, Components(*MassAngleMisTag), FillStyle(3354), FillColor(kGreen+1), ProjectionRange("lowSideband"), Project(RooArgSet(*x,*z)), LineStyle(8), LineColor(kGreen+1), DrawOption("F"));
	}
      ...
      for (unsigned int i = 0; i < 2*nElements; i++)
	{
	  ...
	  else if ((legNames[it] == "Corr. tag sig.") || (legNames[it] == "Mistag sig.")) legLowSideBY->AddEntry(obj,legNames[it++],"PFL");
      ...
      if (GetVar(*TotalPDF,"nSig") != NULL)
	{
	  ...
	  (*TotalPDF)->plotOn(myFrameSignalRegionY, Components(*Signal), FillStyle(3345), FillColor(kBlue), ProjectionRange("signalRegion"), Project(RooArgSet(*x,*z)), LineStyle(7), LineColor(kBlue), DrawOption("F"));
	}
      if (GetVar(*TotalPDF,"nMisTagFrac") != NULL)
	{
	  ...
	  (*TotalPDF)->plotOn(myFrameSignalRegionY, Components(*MassAngleMisTag), FillStyle(3354), FillColor(kGreen+1), ProjectionRange("signalRegion"), Project(RooArgSet(*x,*z)), LineStyle(8), LineColor(kGreen+1), DrawOption("F"));
	}
      ...
      for (unsigned int i = 0; i < 2*nElements; i++)
	{
	  ...
	  else if ((legNames[it] == "Corr. tag sig.") || (legNames[it] == "Mistag sig.")) legSignalRegionY->AddEntry(obj,legNames[it++],"PFL");
      ...
      if (GetVar(*TotalPDF,"nSig") != NULL)
	{
	  ...
	  (*TotalPDF)->plotOn(myFrameHighSideBY, Components(*Signal), FillStyle(3345), FillColor(kBlue), ProjectionRange("highSideband"), Project(RooArgSet(*x,*z)), LineStyle(7), LineColor(kBlue), DrawOption("F"));
	}
      if (GetVar(*TotalPDF,"nMisTagFrac") != NULL)
	{
	  ...
	  (*TotalPDF)->plotOn(myFrameHighSideBY, Components(*MassAngleMisTag), FillStyle(3354), FillColor(kGreen+1), ProjectionRange("highSideband"), Project(RooArgSet(*x,*z)), LineStyle(8), LineColor(kGreen+1), DrawOption("F"));
	}
      ...
      for (unsigned int i = 0; i < 2*nElements; i++)
	{
	  ...
	  else if ((legNames[it] == "Corr. tag sig.") || (legNames[it] == "Mistag sig.")) legHighSideBY->AddEntry(obj,legNames[it++],"PFL");
      ...
      if (GetVar(*TotalPDF,"nSig") != NULL)
	{
	  ...
	  (*TotalPDF)->plotOn(myFrameLowSideBZ, Components(*Signal), FillStyle(3345), FillColor(kBlue), ProjectionRange("lowSideband"), Project(RooArgSet(*x,*y)), LineStyle(7), LineColor(kBlue), DrawOption("F"));
	}
      if (GetVar(*TotalPDF,"nMisTagFrac") != NULL)
	{
	  ...
	  (*TotalPDF)->plotOn(myFrameLowSideBZ, Components(*MassAngleMisTag), FillStyle(3354), FillColor(kGreen+1), ProjectionRange("lowSideband"), Project(RooArgSet(*x,*y)), LineStyle(8), LineColor(kGreen+1), DrawOption("F"));
	}
      ...
      for (unsigned int i = 0; i < 2*nElements; i++)
	{
	  ...
	  else if ((legNames[it] == "Corr. tag sig.") || (legNames[it] == "Mistag sig.")) legLowSideBZ->AddEntry(obj,legNames[it++],"PFL");
      ...
      if (GetVar(*TotalPDF,"nSig") != NULL)
	{
	  ...
	  (*TotalPDF)->plotOn(myFrameSignalRegionZ, Components(*Signal), FillStyle(3345), FillColor(kBlue), ProjectionRange("signalRegion"), Project(RooArgSet(*x,*y)), LineStyle(7), LineColor(kBlue), DrawOption("F"));
	}
      if (GetVar(*TotalPDF,"nMisTagFrac") != NULL)
	{
	  ...
	  (*TotalPDF)->plotOn(myFrameSignalRegionZ, Components(*MassAngleMisTag), FillStyle(3354), FillColor(kGreen+1), ProjectionRange("signalRegion"), Project(RooArgSet(*x,*y)), LineStyle(8), LineColor(kGreen+1), DrawOption("F"));
	}
      ...
      for (unsigned int i = 0; i < 2*nElements; i++)
	{
	  ...
	  else if ((legNames[it] == "Corr. tag sig.") || (legNames[it] == "Mistag sig.")) legSignalRegionZ->AddEntry(obj,legNames[it++],"PFL");
      ...
      if (GetVar(*TotalPDF,"nSig") != NULL)
	{
	  ...
	  (*TotalPDF)->plotOn(myFrameHighSideBZ, Components(*Signal), FillStyle(3345), FillColor(kBlue), ProjectionRange("highSideband"), Project(RooArgSet(*x,*y)), LineStyle(7), LineColor(kBlue), DrawOption("F"));
	}
      if (GetVar(*TotalPDF,"nMisTagFrac") != NULL)
	{
	  ...
	  (*TotalPDF)->plotOn(myFrameHighSideBZ, Components(*MassAngleMisTag), FillStyle(3354), FillColor(kGreen+1), ProjectionRange("highSideband"), Project(RooArgSet(*x,*y)), LineStyle(8), LineColor(kGreen+1), DrawOption("F"));
	}
      ...
      for (unsigned int i = 0; i < 2*nElements; i++)
	{
	  ...
	  else if ((legNames[it] == "Corr. tag sig.") || (legNames[it] == "Mistag sig.")) legHighSideBZ->AddEntry(obj,legNames[it++],"PFL");
				    ...
				    unsigned int ID)
{
  ...
  if (useEffPDF == true)
    {
      ...
      myString << vecParStr->operator[](atoi(Utility->GetGenericParam("NormJPSInotPSIP").c_str()) == true ? Utility->GetJPsiBin(q2Bins) : Utility->GetPsiPBin(q2Bins)).c_str();
      SetValueAndErrors(NULL,"",1.0,&myString,&effPsiGoodTag,&effPsiGoodTagErr,&effPsiGoodTagErr);
      cout << "[ExtractYield::IterativeMass2AnglesFitq2Bins]\t@@@ Integral of S*E over angular variables for normalization channel good-tagged events : " << effPsiGoodTag << " +/- " << effPsiGoodTagErr << " @@@" << endl;
      ...
      myString << vecParStr->operator[](atoi(Utility->GetGenericParam("NormJPSInotPSIP").c_str()) == true ? Utility->GetJPsiBin(q2Bins) : Utility->GetPsiPBin(q2Bins)).c_str();
      SetValueAndErrors(NULL,"",1.0,&myString,&effPsiMisTag,&effPsiMisTagErr,&effPsiMisTagErr);
      cout << "[ExtractYield::IterativeMass2AnglesFitq2Bins]\t@@@ Integral of S*E over angular variables for normalization channel mis-tagged events : " << effPsiMisTag << " +/- " << effPsiMisTagErr << " @@@" << endl;
  ...
  for (unsigned int i = (specBin == -1 ? 0 : specBin); i < (specBin == -1 ? q2Bins->size()-1 : specBin+1); i++)
	  ...
	  (Utility->ValIsInPsi(q2Bins,(q2Bins->operator[](i+1)+q2Bins->operator[](i))/2.) == true))
	{
      ...
      myString.clear(); myString.str("");
      myString << "(mumuMass*mumuMass) > " << q2Bins->operator[](i) << " && (mumuMass*mumuMass) <= " << q2Bins->operator[](i+1);
      cout << "[ExtractYield::IterativeMass2AnglesFitq2Bins]\tCut string: " << myString.str() << endl;
      dataSet_q2Bins[i] = (RooDataSet*)dataSet->reduce(myString.str().c_str());
      ...
      for (int j = 0; j < static_cast<int>(dataSet_q2Bins[i]->sumEntries()); j++)
	{
      ...
      if (CheckGoodFit(fitResult) == true) cout << "[ExtractYield::IterativeMass2AnglesFitq2Bins]\t@@@ Fit converged ! @@@" << endl;
      else                                 cout << "[ExtractYield::IterativeMass2AnglesFitq2Bins]\t@@@ Fit didn't converge ! @@@" << endl;
	  ...
	  EffPDFgoodTag = (RooAbsPdf*)AngleS->clone("EffPDFgoodTag");
      	  ...
      	  cout << "[ExtractYield::IterativeMass2AnglesFitq2Bins]\t===> Integral of S*E over angular variables for signal good-tagged events <===" << endl;
      	  cout << effMuMuGoodTag << "   -" << effMuMuGoodTagErr << "   " << effMuMuGoodTagErr << endl;
	      ...
	      EffPDFmisTag = (RooAbsPdf*)AngleMisTag->clone("EffPDFmisTag");
      	      ...
      	      cout << "[ExtractYield::IterativeMass2AnglesFitq2Bins]\t===> Integral of S*E over angular variables for signal mis-tagged events <===" << endl;
      	      cout << effMuMuMisTag << "   -" << effMuMuMisTagErr << "   " << effMuMuMisTagErr << endl;
      	    }
      	}
      ...
      if (GetVar(TotalPDFq2Bins[i],"nSig") != NULL)
	{
	  ...
	  if ((FitType != 36) && (FitType != 56) && (FitType != 76))
		...
		nEvMisTag = GetVar(TotalPDFq2Bins[i],"nSig")->getVal() / (1. - GetVar(TotalPDFq2Bins[i],"nMisTagFrac")->getVal()) * GetVar(TotalPDFq2Bins[i],"nMisTagFrac")->getVal();
	      ...
	      cout << "[ExtractYield::IterativeMass2AnglesFitq2Bins]\t===> dBF/dq^2 <===" << endl;
...
void MakeMass2AnglesToy (RooAbsPdf* TotalPDF, RooRealVar* x, RooRealVar* y, RooRealVar* z, TCanvas* Canv, unsigned int FitType, unsigned int nToy, int specBin, vector<vector<string>*>* fitParam, RooArgSet* vecConstr, string fileName)
{
  ...
  cout << "[ExtractYield::MakeMass2AnglesToy]\t@@@ I'm generating the toys @@@" << endl;
  if (FULLTOYS == true)
    {
      MyToy->generateAndFit(nToy,nEntryToy,true);
      Canv->Divide(6,5);
  ...
  cout << "[ExtractYield::MakeMass2AnglesToy]\t@@@ Now fit total TOY invariant mass and angles @@@" << endl;
  ...
  for (unsigned int i = 0; i < nToy; i++)
    {
      cout << "[ExtractYield::MakeMass2AnglesToy]\t@@@ Now fitting toy #" << i << " @@@" << endl;
      ...
      do
	{
	  ...
	  if (CheckGoodFit(fitResult) == true) cout << "[ExtractYield::MakeMass2AnglesToy]\t@@@ Fit converged ! @@@" << endl;
	  else
	    {
	      cout << "[ExtractYield::MakeMass2AnglesToy]\t@@@ Fit didn't converge (attempt " << itTrials << "/" << TOYMULTYATTEMPTS << ") ! @@@" << endl;
	      GenerateFitParameters(TotalPDF,fitParam,specBin,"FlAfb_pdf");
	      itTrials++;
	    }
	} while ((CheckGoodFit(fitResult) == false) && (itTrials < TOYMULTYATTEMPTS));
  ...
  if (SAVEPLOT == true)
    {
      cout << "[ExtractYield::MakeMass2AnglesToy]\t@@@ Saving histograms @@@" << endl;
      Canv->Print(fileName.c_str());
    }
...
int main(int argc, char** argv)
{
  if (argc >= 4)
	  ...
	  ((FitType >= 21) && (FitType <= 26) && (argc == 8)))
	{
	  ...
	  if (SETBATCH == true)
	    {
	      cout << "[ExtractYield::main]\t@@@ Setting batch mode @@@" << endl;
	      gROOT->SetBatch(true);
	  ...
	  if (fileFitSystematics.good() == false)
	    {
	      cout << "[ExtractYield::main]\tError opening file : " << myString.str() << endl;
	      CloseAllAndQuit(theApp,NtplFile);
	    }
	  ...
	  if (Utility->WhatIsThis(ParameterFILE) == 0) cout << "[ExtractYield::main]\t@@@ I recognize that this is a DATA file @@@" << endl;
	  else                                         cout << "[ExtractYield::main]\t@@@ I recognize that this is a Monte Carlo file @@@" << endl;
	  ...
	  if (((FitType >= 1)  && (FitType <= 6))  ||
	      ...
	      (FitType == 76))
	    {
	      ...
	      // # Make datasets #
	      // #################
	      cout << "[ExtractYield::main]\t@@@ Making datasets @@@" << endl;
		  ...
		  if (FitType == 2)
		    {
		      ...
		      // ##########################
		      cout << "[ExtractYield::main]\t@@@ Now fit total invariant mass NON resonant channel @@@" << endl;
		      ...
		      cout << "[ExtractYield::main]\t@@@ Now fit total invariant mass resonant J/psi channel @@@" << endl;
		      ...
		      cout << "[ExtractYield::main]\t@@@ Now fit total invariant mass resonant psi(2S) channel @@@" << endl;
	  ...
	  else if ((FitType >= 21) && (FitType <= 26))
	    {
	      ...
	      cout << "[ExtractYield::main]\t@@@ Random seed set to : " << RooRandom::randomGenerator()->GetSeed() << " @@@" << endl;
	      ...
	      if (FitType == 21) // Branching fraction
		{
		  ...
		  // ####################################
		  cout << "[ExtractYield::main]\t@@@ Now make TOY-MC for fit to B0 total invariant mass @@@" << endl;
	      ...
	      else if (FitType == 26) // Fl-Afb-fit
		{
		  ...
		  // ###############################################
		  cout << "[ExtractYield::main]\t@@@ Now make TOY-MC for fit to B0 total invariant mass and cos(theta_K) and cos(theta_l) @@@" << endl;
		  TCanvas* cToyMC = new TCanvas("cToyMC","cToyMC",10,10,1800,1200);
